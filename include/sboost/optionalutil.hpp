...
struct OpJoin {
  ...
  template <typename A>
  struct result<this_type(boost::optional<boost::optional<A>>)> {
...
template <typename A>
boost::optional<A> opAlt(const boost::optional<A>& a,
...
template <typename F, typename A>
auto opMap(F f, const boost::optional<A>& op)
    -> decltype(boost::make_optional(f(*op))) {
  return op ? boost::make_optional(f(*op)) : boost::none;
}

template <typename F, typename A>
typename boost::result_of<F(A)>::type opBind(const boost::optional<A>& a, F f) {
...
boost::optional<A> operator-(const boost::optional<A>& a, const A& b) {
  return a ? boost::make_optional(*a - b) : boost::none;
}
